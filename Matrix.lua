...
if text == 'تفعيل صيح' and Owner(msg) then database:del(bot_id..'Matrix:Seh:User'..msg.chat_id_) send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل امر صيح') return falseendif text == 'تعطيل صيح' and Owner(msg) then database:set(bot_id..'Matrix:Seh:User'..msg.chat_id_,true) send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل امر صيح') return falseendif text and text:match("^صيح @(.*)$") thenlocal username = text:match("^صيح @(.*)$")if not database:get(bot_id..'Matrix:Seh:User'..msg.chat_id_) thenfunction start_function(extra, result, success)if result and result.message_ and result.message_ == "USERNAME_NOT_OCCUPIED" then send(msg.chat_id_, msg.id_,'⌔︙المعرف غلط ') return false endif result and result.type_ and result.type_.channel_ and result.type_.channel_.ID == "Channel" thensend(msg.chat_id_, msg.id_,'⌔︙لا اسطيع صيح معرفات القنوات') return false endif result.type_.user_.type_.ID == "UserTypeBot" thensend(msg.chat_id_, msg.id_,'⌔︙لا اسطيع صيح معرفات البوتات') return false endif result and result.type_ and result.type_.channel_ and result.type_.channel_.is_supergroup_ == true thensend(msg.chat_id_, msg.id_,'⌔︙لا اسطيع صيح معرفات المجموعات') return false endif result.id_ thensend(msg.chat_id_, msg.id_,'⌔︙تعال حبي يصيحونك بل كروب [@'..username..']') return falseendendtdcli_function ({ID = "SearchPublicChat",username_ = username}, start_function, nil)elsesend(msg.chat_id_, msg.id_,'⌔︙امر صيح تم تعطيله من قبل المدراء ') endreturn falseend end
if text == "صورتي"  then
